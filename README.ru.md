...
> [!IMPORTANT]
> - Поддержка рантайма: **Linux (стабильно)** и **Windows (экспериментально)**.
> - Сборка под Windows поддерживается в экспериментальном режиме через WinAPI backend чтения памяти процессов.
> - Для запуска генерации нужны повышенные права: на Linux используйте **root** (`sudo`) для чтения `/proc/<pid>/mem` и `/proc/<pid>/maps`; на Windows запускайте терминал от имени **администратора**.
> - По умолчанию CLI/help/log выводятся на **английском**. Для русского режима используйте `--lang ru`.
...
Совместимая сборка под Windows:

- CMake 3.16 или новее.
- MSVC (Visual Studio 2019+) или другой toolchain с C++17.
- Для доступа к памяти большинства процессов запускайте терминал от имени администратора.
...
- `BUILD_TYPE` (по умолчанию `Release`)
- `BUILD_DIR` (по умолчанию `build/linux-<build_type>`)
- `OUT_DIR` (по умолчанию `dist/linux`)
- `RUN_TESTS` (по умолчанию `1`, поставьте `0`, чтобы пропустить)
- `JOBS` (количество параллельных задач сборки)
...
- Скачивает и распаковывает `llvm-mingw`, `cmake` и `ninja` в `.tools/windows` внутри проекта.
- Собирает через CMake + Ninja.
- По умолчанию запускает тесты.
- Копирует runtime DLL рядом с `ram_audio.exe` в `dist/windows`.

Артефакты:

- `dist/windows/ram_audio.exe`
- `dist/windows/ram_audio_telemetry_test.exe`
- нужные runtime DLL (`libc++.dll`, `libunwind.dll` и другие при необходимости)
...
Базовый шаблон команды для Linux:

```bash
sudo ./build/ram_audio [options]
```

Шаблон команды для Windows (PowerShell, запуск от имени администратора):

```powershell
.\dist\windows\ram_audio.exe [options]
```

### Быстрые примеры для Windows
...
Опциональный стрим-предпросмотр (нужен `ffplay` в PATH):
...
- `--lang` (`en` или `ru`, по умолчанию `en`) - язык CLI/help/log.
